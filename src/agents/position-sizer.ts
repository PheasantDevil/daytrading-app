...
export class PositionSizer {
  ...
  calculateFixedRisk(
    ...
    return {
      ...
      riskPercent: (finalSize * riskPerShare / this.config.accountBalance) * 100,
      method: 'Fixed Risk',
      confidence: 85,
    };
  }
  ...
  calculateKellyCriterion(
    ...
    const kellyPercent = (winRateDecimal * averageWin - lossRateDecimal * averageLoss) / averageWin;
  ...
  calculateRiskParity(
    ...
    const availableRisk = Math.min(
      targetRisk,
      this.config.accountBalance * (this.config.maxPortfolioRisk / 100) - portfolioRisk
    );
    
    const positionSize = Math.floor(availableRisk / riskPerShare);
    const riskContribution = (positionSize * riskPerShare / this.config.accountBalance) * 100;
  ...
  calculateIntegrated(
    ...
    const volatilityBased = this.calculateVolatilityBased(entryPrice, volatility);
    const kelly = this.calculateKellyCriterion(winRate, averageWin, averageLoss, entryPrice);
    ...
    if (kelly.isRecommended) {
      ...
      recommendedSize = Math.floor(
        fixedRisk.recommendedSize * 0.6 +
        volatilityBased.recommendedSize * 0.4
      );
      confidence = 75;
    }
    ...
    // ポートフォリオリスク制限を適用
    const riskPerShare = Math.abs(entryPrice - stopLossPrice);
    const maxRiskAmount = this.config.accountBalance * (this.config.maxPortfolioRisk / 100) - portfolioRisk;
    ...
    return {
      ...
      riskPercent: (recommendedSize * riskPerShare / this.config.accountBalance) * 100,
      method: 'Integrated',
      confidence,
    };
  }
  ...
  analyzeRisk(
    ...
    const portfolioRiskAfter = portfolioRisk + (positionRisk / this.config.accountBalance) * 100;
