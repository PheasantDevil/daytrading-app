...
export default function SecuritiesAccountForm({
  ...
  const validateForm = (): boolean => {
    ...
    if (!formData.apiKey) newErrors.apiKey = 'APIキーは必須です';
    if (!formData.apiSecret) newErrors.apiSecret = 'APIシークレットは必須です';
    ...
    if (!formData.tradingPassword)
      newErrors.tradingPassword = '取引パスワードは必須です';
  ...
  const togglePasswordVisibility = (field: 'password' | 'tradingPassword') => {
      ...
      <form onSubmit={handleSubmit} className="space-y-6">
        ...
        <div>
          ...
          <input
            ...
            onChange={handleInputChange}
            placeholder="SBI証券開発者ポータルで取得したAPIキー"
            className={`w-full px-3 py-2 border rounded-md focus:outline-none focus:ring-2 focus:ring-blue-500 ${
        ...
        <div>
          ...
          <input
            ...
            onChange={handleInputChange}
            placeholder="SBI証券開発者ポータルで取得したAPIシークレット"
            className={`w-full px-3 py-2 border rounded-md focus:outline-none focus:ring-2 focus:ring-blue-500 ${
        ...
        <div>
          ...
          <div className="relative">
            <input
              type={showPasswords.tradingPassword ? 'text' : 'password'}
              id="tradingPassword"
              name="tradingPassword"
              value={formData.tradingPassword}
              onChange={handleInputChange}
              placeholder="取引時に使用するパスワード"
              className={`w-full px-3 py-2 pr-10 border rounded-md focus:outline-none focus:ring-2 focus:ring-blue-500 ${
                errors.tradingPassword ? 'border-red-500' : 'border-gray-300'
              }`}
              disabled={isLoading}
            />
            <button
              type="button"
              onClick={() => togglePasswordVisibility('tradingPassword')}
              className="absolute inset-y-0 right-0 pr-3 flex items-center"
              disabled={isLoading}
            >
              {showPasswords.tradingPassword ? (
                <svg
                  className="h-5 w-5 text-gray-400"
                  fill="none"
                  viewBox="0 0 24 24"
