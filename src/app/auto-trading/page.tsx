...
export default function AutoTradingPage() {
  ...
  const [strategies, setStrategies] = useState<Strategy[]>([
    { id: 'momentum', name: 'モメンタム', enabled: true, description: '価格の勢いを利用した戦略' },
    { id: 'mean-reversion', name: '平均回帰', enabled: false, description: '価格の平均回帰を利用した戦略' },
    { id: 'breakout', name: 'ブレイクアウト', enabled: true, description: '価格の突破を利用した戦略' },
  ]);
  ...
  const [positions, setPositions] = useState<Position[]>([
    { symbol: '7203.T', name: 'トヨタ自動車', quantity: 100, entryPrice: 2500, currentPrice: 2525, profit: 2500 },
    { symbol: '6758.T', name: 'ソニーグループ', quantity: 50, entryPrice: 12400, currentPrice: 12376, profit: -1200 },
  ]);

  const [tradeLogs, setTradeLogs] = useState<TradeLog[]>([
    { timestamp: '12:34:56', symbol: '7203.T', action: 'buy', quantity: 100, price: 2500 },
    { timestamp: '12:45:23', symbol: '6758.T', action: 'sell', quantity: 50, price: 12376, profit: -1200 },
    { timestamp: '13:12:45', symbol: '7203.T', action: 'sell', quantity: 100, price: 2525, profit: 2500 },
  ]);
  ...
  return (
    <DashboardLayout>
        ...
        <div className="bg-white rounded-lg shadow-md p-6">
          <div className="flex items-center justify-between mb-4">
            ...
            <div className="flex items-center space-x-4">
              <div className={`px-3 py-1 rounded-full text-sm font-medium ${
                ...
                {status.isRunning && !status.isPaused ? '● 実行中' : 
                 status.isPaused ? '● 一時停止' : '● 停止中'}
              </div>
            </div>
          </div>
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
            <div className="text-center">
              ...
              <p className="text-2xl font-bold text-gray-900">{status.todayTrades}回</p>
            </div>
            <div className="text-center">
              <p className="text-sm text-gray-500">今日の利益</p>
              <p className={`text-2xl font-bold ${status.todayProfit >= 0 ? 'text-green-600' : 'text-red-600'}`}>
                {status.todayProfit >= 0 ? '+' : ''}¥{status.todayProfit.toLocaleString()}
              </p>
            </div>
            <div className="text-center">
              <p className="text-sm text-gray-500">アクティブポジション</p>
              <p className="text-2xl font-bold text-gray-900">{positions.length}件</p>
            </div>
          </div>
          ...
          <div className="space-y-4">
            <div>
              ...
              <input
                ...
                onChange={(e) => handleRiskSettingChange('maxDailyLoss', Number(e.target.value))}
            ...
            <div>
              ...
              <input
                ...
                onChange={(e) => handleRiskSettingChange('positionSizePercent', Number(e.target.value))}
                className="w-full px-3 py-2 border border-gray-300 rounded-md focus:outline-none focus:ring-2 focus:ring-blue-500"
              />
            </div>
            <div>
              ...
              <input
                ...
                onChange={(e) => handleRiskSettingChange('stopLossPercent', Number(e.target.value))}
                className="w-full px-3 py-2 border border-gray-300 rounded-md focus:outline-none focus:ring-2 focus:ring-blue-500"
              />
            </div>
            <div>
              ...
              <input
                ...
                onChange={(e) => handleRiskSettingChange('takeProfitPercent', Number(e.target.value))}
                className="w-full px-3 py-2 border border-gray-300 rounded-md focus:outline-none focus:ring-2 focus:ring-blue-500"
              />
            </div>
          </div>
            ...
            <table className="min-w-full divide-y divide-gray-200">
              <thead className="bg-gray-50">
                <tr>
                  ...
                  <th className="px-6 py-3 text-left text-xs font-medium text-gray-500 uppercase tracking-wider">取得価格</th>
      ...
      <div className="mt-6">
        <div className="bg-white rounded-lg shadow-md p-6">
          <h3 className="text-lg font-semibold mb-4">取引ログ（リアルタイム）</h3>
            ...
            {tradeLogs.map((log, index) => (
              <div key={index} className="flex justify-between items-center p-3 bg-gray-50 rounded">
                <div className="flex items-center space-x-4">
                  ...
                  <span className="text-sm">{log.quantity}株 @¥{log.price.toLocaleString()}</span>
                </div>
