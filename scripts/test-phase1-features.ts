...
import { MultiTimeframePredictor } from '../src/ml/multi-timeframe-predictor';
...
async function testRiskManager(): Promise<void> {
  ...
  try {
    ...
    // ポジションサイジング
    const positionSize = riskManager.calculatePositionSize(1000000, 1000, 950, 2);
    ...
    console.log(`✅ ポートフォリオリスク: ${portfolioRisk.riskPercentage.toFixed(2)}%`);
...
async function testTechnicalAnalyzer(): Promise<void> {
  ...
  try {
    ...
    console.log(`✅ ボリンジャーバンド幅: ${indicators.bollinger.width.toFixed(4)}`);
...
async function testPositionSizer(): Promise<void> {
  ...
  try {
    ...
    console.log(`✅ ポジション価値: ${fixedRisk.positionValue.toLocaleString()}円`);
    ...
    // 統合ポジションサイジング
    const integrated = positionSizer.calculateIntegrated(1000, 950, 15, 60, 5000, 3000, 5);
...
async function testMultiStockMonitor(): Promise<void> {
  ...
  try {
    ...
    analyses.forEach(analysis => {
      console.log(`  - ${analysis.symbol}: ${analysis.recommendation} (信頼度: ${analysis.confidence.toFixed(2)}%)`);
    });
    
    // ポートフォリオ分析
    const portfolioAnalysis = await monitor.analyzePortfolio();
    console.log(`✅ ポートフォリオ総価値: ${portfolioAnalysis.totalValue.toLocaleString()}円`);
...
async function testBacktestEngine(): Promise<void> {
  ...
  try {
    ...
    console.log(`✅ 総リターン: ${result.performance.totalReturnPercent.toFixed(2)}%`);
    console.log(`✅ 最大ドローダウン: ${result.performance.maxDrawdown.toFixed(2)}%`);
